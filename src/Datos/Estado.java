...
public class Estado
{
    ...
    /**
     * Constructor for objects of class Estado
     * @param bd
     */
    public Estado(Conexion bd)
    {  
        this.bd = bd;
        Inicializar();
    } 
    ...
    public ResultSet DataSetEstado() throws SQLException
    {
        String sql = "SELECT id AS ID, nombre AS Nombre FROM Estado ORDER BY nombre";
        this.bd.ejecutarSQL(sql);        
        return(this.bd.resultset);
    }

    public ResultSet DataSetEstado(String patron, int campo)
    {
        String sql;
        sql = null;
        switch (campo)
        {
            case 0: {sql = "SELECT id AS ID, nombre AS Nombre FROM Estado "+
                           "WHERE id="+Integer.valueOf(patron) + " ORDER BY nombre"; break;}
            case 1: {sql = "SELECT id AS ID, nombre AS Nombre FROM Estado "+
                           "WHERE nombre LIKE '%"+ patron.toUpperCase()+"%'" + " ORDER BY nombre"; break;}
        }        
        this.bd.ejecutarSQL(sql);
        return(this.bd.resultset);
    } 
    ...
    public boolean Nuevo()
    {
        ...
        {
            String orden;                       
            orden = "INSERT INTO Estado(id,nombre) VALUES(?,?)";
    ...
    {        
        try
        {
            String orden = "UPDATE Estado SET nombre=? WHERE id = ?";  
    ...
    public boolean Eliminar()
    {
        try
        {
            String orden = "DELETE FROM Estado WHERE id = ?";
      ...
      try
       {
        ...
        {
          // Se introduce el catalogo de grupos como objetos (ver clase ListaEstados)
